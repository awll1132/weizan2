<?php
/**
 *源码来自鬼狐源码社区www.guifox.com【本站唯一淘宝店：https://shop100181238.taobao.com】其他店铺购买均为盗用本站
 */
...
if($do == 'display') {
	...
	if (checksubmit('submit')) {
		...
		$data = array(
			...
			'template' => mysql_real_escape_string($_GPC['template']),
			'title' => mysql_real_escape_string($_GPC['title']),
			'description' => mysql_real_escape_string($_GPC['description']),
			'content' => htmlspecialchars_decode($_GPC['content'], ENT_QUOTES),
			'incontent' => intval($_GPC['incontent']),
			'source' => mysql_real_escape_string($_GPC['source']),
			'author' => mysql_real_escape_string($_GPC['author']),
