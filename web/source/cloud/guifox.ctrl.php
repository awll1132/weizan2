<?php
/**
 * [WDL] Copyright (c) 2013 www.guifox.com
 * 鬼狐社区微赞一键更新程序
 */
load()->func('file');
defined('IN_IA') or exit('Access Denied');
    ...
    $ver = include('guifox.php');
    $ver = $ver['ver'];
    $ver = substr($ver,-4);
	$hosturl = urlencode('http://' . $_SERVER['HTTP_HOST']);
	$updatehost = 'http://wzsq.guifox.com/update.php';
	...
	elseif($_GPC['op'] == 'update'){
		...
		if (strstr($updatenowinfo, 'zip')){			
				...
				if($isgot){				
					...
					$f = fopen('guifox.php','w+');
					fwrite($f,$newver);
					fclose($f);
					@unlink(IA_ROOT . '/update.sql');
					deldir($updatedir);
					
					message('已将系统更新到最新版本！',create_url('cloud/guifox',array('op'=>'display')),'success');
				}
				else{
				...
				message('鬼狐社区提醒您:请检查授权状态，或者联系鬼狐社区授权！',referer(),'error');
		}
	}
