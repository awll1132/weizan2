...
class Lonaking_super_questionModuleSite extends WeModuleSite {
	...
	public function doWebTeamManage(){
		...
		if($page['count'] > 0){
			$captain_ids = FlashHelper::fetchModelArrayIds($page['data'],'captain_id');
	...
	public function doWebRecordManage(){
		...
		if(!empty($_GPC['nickname'])){
			$pWhere = " AND p.nickname like '%{$_GPC['nickname']}%'";
		}
		$page_index = max(1, intval($_GPC['page']));
		...
		$tmpData = pdo_fetchall("select r.id,r.uniacid,r.record_number,r.type,r.player_id,r.openid,r.uid,r.is_captain,r.right,r.wrong,r.right_ids,r.wrong_ids,r.score,r.activity_id,r.answer_seconds,r.shared,r.is_help,r.help_record_id,r.create_time,r.update_time,a.name activity_name,a.limit_seconds,p.headimgurl,p.nickname FROM ".tablename($this->recordService->table_name) ." r LEFT JOIN ".tablename($this->activityService->table_name)." a ON r.activity_id=a.id LEFT JOIN ".tablename($this->playerService->table_name)." p ON r.player_id=p.id WHERE r.uniacid={$_W['uniacid']} AND 1=1 {$pWhere} ORDER BY {$orderBy}r.id DESC LIMIT {$limit} ");
		$tmpCount = $this->recordService->count();
		$tmpCount = pdo_fetchcolumn("select count(1) FROM ".tablename($this->recordService->table_name) ." r LEFT JOIN ".tablename($this->activityService->table_name)." a ON r.activity_id=a.id LEFT JOIN ".tablename($this->playerService->table_name)." p ON r.player_id=p.id WHERE r.uniacid={$_W['uniacid']} AND 1=1 {$pWhere}");
	...
	public function doMobileIndex() {
		...
		try{
			load()->model('mc');
			$user = mc_oauth_userinfo($this->activityService->getUniacid());
			$player = $this->playerService->checkPlayerRegister();
			$this->activityService->log($player,"第二步,检测用户是否注册成功");
			$nickname = $player['nickname'];

			$this->playerService->log($player,"get player info success , player info is :");
			$activity = $this->activityService->getActivityAndRandomQuestionsById($_GPC['id']);

			$this->activityService->log($activity,"第三步,获取到活动信息为");
			$urls = array(
			...
			//$html['follow'] = $this->checkFollow($player['openid'],$_W['uniacid'],$this->module['config']['oauth_borrow']);
			//share content
			$share = array(
	...
	public function doMobileCheckPlayLimit(){
		...
		try{
			...
			//判断活动开始结束
			$this->checkActivityIsIng($activity);
	...
	private function checkFollow($openid,$uniacid,$borrowOauth = 0){
		load()->model('mc');
		$follow = 0;
		if($borrowOauth == 0){
			$fansInfo = mc_fansinfo($openid, $this->getUniacid());
			$follow = $fansInfo['follow'];
		}elseif($borrowOauth == 1){
			//借权的公众号对应的信息
			$fansInfo = mc_fansinfo($openid,$this->getUniacid());
			//不借权的
			if(trim($fansInfo['openid']) != "" && $fansInfo['openid'] != $fansInfo['tag']['openid'] && !empty($fansInfo['tag']['openid']) && $fansInfo['follow'] == 1){
				return 1;
			}
			$nickname = $fansInfo['nickname'];
			if(empty($nickname) || trim($nickname) == ""){
				$nickname = $fansInfo['tag']['nickname'];
			}
			$existUser = pdo_fetch("select * from ".tablename("mc_mapping_fans")." where nickname like '%{$nickname}%' and uniacid={$uniacid}");
			if(empty($existUser)){
				$follow = 0;
			}else{
				$follow = $existUser['follow'];
			}
		}
		return $follow;
	}
	...
	public function doMobileRecordNote(){
		...
		try{
			...
			$record = array(
				...
				'uid' =>$uid,
				'is_captain' => 0,
				'right' => intval($_GPC['right']),//答错
				'wrong' => intval($_GPC['wrong']),//答错
			...
			$gRightSize = sizeof($_GPC['right_ids']);
			$gWrongSize = sizeof($_GPC['wrong_ids']);

			$limit = $this->recordService->checkPlayLimit($openid,$activityId,$activity['play_limit'],$activity['limit_type']);
			//限制提交次数
			if($limit['limit'] <= 0){
				return $this->return_json(400,"参与次数超限制，感谢您的参与！",null);
			}
	...
	private function checkActivityIsIng($activity){
		if(strtotime($activity['start_time']) > time()  ){
			throw new Exception("活动还未开始,不要着急哦",400);
		}
		if(strtotime($activity['end_time']) < time()){
			throw new Exception("活动已经结束了,感谢您的关注",400);
		}
		if($activity['score'] <= 0){
			throw new Exception("活动已经结束了,感谢您的关注!",400);
		}
	...
	public function doMobileHelpRecordNote(){
		...
		try{
			...
			$limit = $this->recordService->checkPlayLimit($openid,$activityId,$activity['help_limit'],$activity['limit_type'],2);
			//限制提交次数
			if($limit['limit'] <= 0){
				return $this->return_json(400,"每个活动仅限制{$limit['count']}人帮助答题",null);
			}
	...
	public function doMobileActivityRankPage(){
		...
		if(empty($record)){
			return message("您没有合法的游戏记录,点击参与游戏",$this->createMobileUrl("Index"),"error");
		}
		$user['rank'] = $this->recordService->rankOne($record['id'],"AND a.max=1 AND a.activity_id={$activity['id']} ORDER BY a.score DESC,a.answer_seconds ASC,a.create_time DESC");
		$html = array(
			'activity' => $activity,
			'user' => $user,
			'record' => $record
	...
	public function doMobilePersonRankPage(){
		global $_W;
		$user =  $this->playerService->selectPlayerByOpenid($_W['openid']);
		if(empty($user['nickname']) && !empty($user['id'])){
			$fansInfo = $this->flashUserService->fetchFansInfo($_W['openid']);
			$user['nickname'] = $fansInfo['tag']['nickname'];
			$user['headimgurl'] = $fansInfo['tag']['avatar'];
			$user = $this->playerService->updateData($user);
		}
		$user['rank'] = $this->playerService->rankOne($user['id'],"AND a.play_times>=1 AND a.nickname!='' ORDER BY a.total_score DESC");
		$html = array(
			'user' => $user,
		);
		$urls = array(
	...
	public function doMobilePersonRank(){
		$page = $this->playerService->selectPageOrderBy("and play_times>=1 and nickname!=''","total_score DESC,play_times DESC,");
		return $this->return_json(200,'success',$page);
	}

	public function doMobileTeamAddPage(){
		global $_W,$_GPC;
		$user = $this->flashUserService->fetchFansInfo($_W['openid']);

		$teamAddApi = $this->createMobileUrl("TeamAdd");
		include $this->template("team-add");
	}

	public function doMobileTeamList(){
		global $_W,$_GPC;
		$teams = $this->teamService->selectAll();
		$player = $this->playerService->checkPlayerRegister();
		if(!empty($player) && $player['team_id'] != 0){
			$player['team'] = $this->teamService->selectById($player['team_id']);
		}
	...
	public function doMobileMyTeam(){
		...
		$player = $this->playerService->checkPlayerRegister();
		$data['name'] = $player['nickname']."的战队";
		$data['member_count'] = 1;
		$data['uniacid'] = $_W['uniacid'];
		$data['captain_id'] = $player['id'];
		$data['activity_times'] = 0;
		$data['score'] = 0;
		$data['question_count'] = 0;
		$data['logo'] = $player['headimgurl'];
		$data['start_time'] = time();
		$data['create_time'] = time();
		$data['update_time'] = time();
		$team = $this->teamService->insertData($data);
		return message("战队创建成功，跳转中...",$this->createMobileUrl("MyTeam"),"success");
	}
	...
	public function doMobileTeamJoin(){
		...
		$player = $this->playerService->selectPlayerByOpenid($_W['openid']);
		if($player['team_id'] > 0){
			$old_team = $this->teamService->selectById($player['team_id']);
			$old_team['member_count'] = $old_team['member_count'] - 1;
			$this->teamService->updateData($old_team);
		}
		$captain = $this->playerService->selectById($team['team_captain']);
		//TODO here should notice the captain with template notice.
		$this->playerService->updateColumn('team_id',$team['id'],$player['id']);
		$this->teamService->columnAddCount('member_count',1,$team['id']);
		return $this->return_json(200,'success,',null);
	}
	...
	public function doWebHeadImageFix(){
		load()->model('mc');
		$noHeadPlayerList = $this->playerService->selectAll("AND headimgurl=''");
		//$fansInfo = mc_fansinfo($openid);
		$uidIds = array();
		foreach($noHeadPlayerList as $player){
			$uidIds[] = $player['uid'];
		}
		$uidIdsStr = implode(",",$uidIds);
		$members = pdo_fetchall("SELECT * FROM ims_mc_members WHERE uid in ({$uidIdsStr})");
	...
	private function htmlStatic(){
		global $_W;
		$arr = array(
			'prefix' => '../addons/lonaking_super_question/template/mobile/',
			'urls' => array(

			),

			'js' => $_W['account']['jssdkconfig'],
		);
		return $arr;
	}

	public function doWebQuestionCopy(){
		global $_W,$_GPC;
		$source = $_W['uniacid'];
		$target = $_GPC['target'];

		$all = $this->questionService->selectAll();
		$index = 0;
		foreach ($all as $q) {
			$q['uniacid'] = $target;
			//$this->questionService->insertData()
			$q['id'] = null;
			pdo_insert($this->questionService->table_name,$q);
			$index = $index + 1;
		}
		echo "完成,共复制{$index}条数据";
	}
}