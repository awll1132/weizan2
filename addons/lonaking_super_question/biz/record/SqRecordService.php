...
class SqRecordService extends FlashCommonService
{
     ...
     * @param int $limit
     * @param int $limitType
     * @param int $playType 1 主动 2帮助
     * @return int
     */
    public function checkPlayLimit($openid,$activityId,$limit = 1,$limitType = 1,$playType = 1){
        $count = 0;
        if($playType == 1){
            if($limitType == 1){// 天
            ...
            //help 不按天 不按活动
            $count = $this->count("AND openid='{$openid}' AND activity_id={$activityId} and is_help=1");
            $limit = ($limit - $count > 0) ? $limit-$count : 0;
            return array(
                'limit' => $limit,
                'count' => $count
