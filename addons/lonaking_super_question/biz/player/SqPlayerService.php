...
class SqPlayerService extends FlashCommonService
{
    ...
    public function __construct()
    {
        ...
        $this->columns = 'id,uniacid,openid,nickname,headimgurl,uid,play_times,share_times,total_score,team_id,create_time,update_time';
...
//         }
    ...
    public function initPlayerInfo(){
        global $_W;
        $fansInfo = $this->flashUserService->authFansInfo();

        $player = array(
            'uniacid' => $_W['uniacid'],
            'openid' => $_W['openid'],
            'nickname' => $fansInfo['tag']['nickname'],
            'headimgurl' => $fansInfo['tag']['avatar'],
            'uid' => $fansInfo['uid'],
            'play_times' => 0,
            'total_score' => 0,
            'team_id' => 0,
    ...
    public function checkPlayerRegister(){
        global $_W;
        $player = $this->selectOne("AND uniacid={$_W['uniacid']} AND openid='{$_W['openid']}'");
        if(empty($player)){
            $tmpPlayer = $this->initPlayerInfo();
            $this->log($tmpPlayer, "临时tmpPlayer===");
            $player = $this->checkPlayerRegister();

            $this->log($player,".............");
            return $player;
        }
        $this->log($player,"查找到用户信息======");
        $fansInfo = $this->flashUserService->fetchFansInfo($_W['openid']);
        $this->log($fansInfo,"查找到fansInfo=====");
        $player['fans_info'] = $fansInfo;
        return $player;
