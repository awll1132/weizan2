...
class J_moneyModule extends WeModule {
	...
	public function fieldsFormSubmit($rid) {
		...
		if(!$_GPC['gameid'])message("请选择营销活动");
		if (empty($reply_id)) {
			$insert['rid']=$rid;
			$insert['status']=1;
	...
	public function settingsDisplay($settings) {
		...
		if (checksubmit()) {
			...
			$cfg = array(
				...
				'printnum' => intval($_GPC['printnum'])>9 ? 9 : intval($_GPC['printnum']),
				'bg' => $_GPC['bg'],
				'printmachine' => $_GPC['printmachine'],
				
				'login_pc_type' => intval($_GPC['login_pc_type']),
				'login_m_type' => intval($_GPC['login_m_type']),
				/*支付宝参数*/
