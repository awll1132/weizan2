...
function getAccessToken($id){
	if(!$id)return false;
	$item = pdo_fetch("SELECT * FROM ".tablename('j_money_group')." WHERE id = :a", array(':a' => $id));
	if(!$item['token'] || !$item['expire_time'] || TIMESTAMP>$item['expire_time']){
		if(!$item['appid'] || !$item['appsecret'])return false;
		load()->func('communication');
		$urls = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=".$item['appid']."&secret=".$item['appsecret']."";
		...
		$data['expire_time'] = TIMESTAMP + $result['expires_in'];
		pdo_update('j_money_group', $data, array('id' => $id));
		return $data['token'];
	}else{
		if(TIMESTAMP<$item['expire_time'])return $token;
	}
}

function trueUrl2Shorturl($longurl){
	global $_W;
	load()->func('communication');
	$acid=pdo_fetchcolumn("SELECT acid FROM ".tablename('account')." WHERE uniacid=:uniacid ",array(':uniacid'=>$_W['uniacid']));
	$acc = WeAccount::create($acid);
	$tokens=$acc->fetch_token();
	$url = "https://api.weixin.qq.com/cgi-bin/shorturl?access_token=".$tokens;
	$data=array(
		"action"=>"long2short",
		"long_url"=>$longurl,
	);
	...
	if(is_error($content))return false;
	$result = @json_decode($content['content'], true);
	if(empty($result) || !is_array($result) || $result['errcode']!=0)return false;
